 ...
 */
class DefaultAggregateRootInternalHandlerProvider : AssemblyInitializer {
    override fun initialize(objectContainer: ObjectContainer, componentTypes: Set<Class<*>>) {
        componentTypes.filter { type -> TypeUtils.isAggregateRoot(type) }.forEach { aggregateRootType ->
            recurseRegisterInternalHandler(
                aggregateRootType
            )
        }
    }
    ...
    private fun register(aggregateRootType: Class<*>, type: Class<*>) {
        Arrays.stream(type.declaredMethods).filter { method: Method ->
            method.name.startsWith(SysProperties.AGGREGATE_ROOT_HANDLE_METHOD_NAME_PREFIX) && method.parameterTypes.size == 1 && DomainEventMessage::class.java.isAssignableFrom(
    ...
    private fun registerInternalHandler(aggregateRootType: Class<*>, eventType: Class<*>, method: Method) {
        ...
        eventHandlerDic[eventType] = Action2 { aggregateRoot: AggregateRoot, domainEventMessage: DomainEventMessage ->
            methodHandle.invoke(aggregateRoot, domainEventMessage)
        }
    }

    companion object Dict {
        @JvmField
        val aggregateRootInternalHandlerMap: MutableMap<Class<*>, MutableMap<Class<*>, Action2<AggregateRoot, DomainEventMessage>>> =
            HashMap()

        @JvmStatic
        fun getInternalEventHandler(
            aggregateRootType: Class<out AggregateRoot>, eventType: Class<out DomainEventMessage>
        ): Action2<AggregateRoot, DomainEventMessage> {
            var currentAggregateType = aggregateRootType
            while (true) {
                val handler = aggregateRootInternalHandlerMap[currentAggregateType]?.get(
                    eventType
                )
                if (handler != null) {
                    return handler
                }
                if (currentAggregateType.superclass != null && listOf(*currentAggregateType.superclass.interfaces).contains(
                        AggregateRoot::class.java
                    )
                ) {
