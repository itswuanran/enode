...
class MysqlEventStore(
    ...
    private fun batchAppendAggregateEvents(
        ...
        client.withTransaction { client -> client.preparedQuery(sql).executeBatch(batch) }.onComplete { ar ->
            if (ar.succeeded()) {
    ...
    private fun findByVersion(aggregateRootId: String, version: Int): CompletableFuture<DomainEventStream?> {
        ...
        client.preparedQuery(sql).execute(Tuple.of(aggregateRootId, version)).onComplete { ar ->
            if (ar.succeeded()) {
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(MysqlEventStore::class.java)
