...
public class RedisSendReplyService implements SendReplyService {
    ...
    public CompletableFuture<SendMessageResult> send(ReplyMessage message) {
        String queueName = String.format("EQUEUE/%s", message.getAddress());
        return redissonReactiveClient.getQueue(queueName).add(message).toFuture().thenApply(x -> {
                return new SendMessageResult(message.getId(), Maps.newHashMap());
            }
        );
    }
}
